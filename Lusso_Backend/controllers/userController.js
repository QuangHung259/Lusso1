...
const registerUser = async (req, res) => {
  try {
    const { firstName, lastName, email, password, phone } = req.body; // ✅ thay fullName thành firstName & lastName
...
const loginUser = async (req, res) => {
  try {
    ...
    res.json({
      ...
      user: {
        _id: user._id,
        fullName: `${user.firstName} ${user.lastName}`, // ghép
        email: user.email,
        role: user.role,
      },
    });
...
const updateUserProfile = async (req, res) => {
  try {
    ...
    const { firstName, lastName, phone, address } = req.body; // ✅ tách ra
